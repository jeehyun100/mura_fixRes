...
class Trainer:
    ...
    def _init_state_test(self) -> None:
        ...
        optimizer = optim.RMSprop(model.parameters(), lr=self._train_cfg.lr, weight_decay=1e-8, momentum=0.9)
        lr_scheduler = optim.lr_scheduler.ReduceLROnPlateau(optimizer, 'min' if model.n_classes > 1 else 'max', patience=2)
    ...
    def _train(self) -> Optional[float]:
        ...
        if self._state.model.n_classes > 1:
    ...
    def _test(self) -> Optional[float]:
        ...
        if self._train_cfg.mura_part.lower() == 'all':
