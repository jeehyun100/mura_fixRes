        ...
        if transforms is None:
            ...
            if self.train and not self.test:
                ...
                self.transforms = albumentations.Compose([
                    ...
                    albumentations.Resize(320),
                    albumentations.RandomCrop(320),
                    #T.RandomHorizontalFlip(),
                    albumentations.RandomVerticalFlip(),
                    albumentations.RandomRotation(30),
                    albumentations.ToTensor(),
                    albumentations.Lambda(lambda x: t.cat([x[0].unsqueeze(0), x[0].unsqueeze(0), x[0].unsqueeze(0)], 0)),  # 转换成3 channel
                    albumentations.Normalize(mean=MURA_MEAN, std=MURA_STD),
                ])
            if not self.train:
                # 这里的X光图是1 channel的灰度图
                self.transforms = albumentations.Compose([
                    # T.Lambda(logo_filter),
                    albumentations.Resize(400),
                    albumentations.CenterCrop(320),
                    albumentations.ToTensor(),
                    albumentations.Lambda(lambda x: t.cat([x[0].unsqueeze(0), x[0].unsqueeze(0), x[0].unsqueeze(0)], 0)),  # 转换成3 channel
                    albumentations.Normalize(mean=MURA_MEAN, std=MURA_STD),
                ])
