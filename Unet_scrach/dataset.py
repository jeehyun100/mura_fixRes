...
class MURA_Dataset(object):
        ...
        with open(csv_path, 'rb') as F:
            ...
            if part.lower() == 'all':
                ...
                imgs = [root + str(x, encoding='utf-8').strip() for x in d if
                        str(x, encoding='utf-8').strip().split('/')[2] == part][:20]
    ...
    def __getitem__(self, index):
        ...
        clahe = cv2.createCLAHE(clipLimit=1, tileGridSize=(7, 7))
        clahe_img = clahe.apply(img_croped)
        ...
        morpology_img = np.dstack((clahe_img,opening_img, closing_img))

        data = Image.fromarray(morpology_img)
